...
class TestBayesianOnlineAlgorithm:
    ...
    def test_online_detection_correctness(self, inner_algorithm_factory, generate_data, data_params):
        for _ in range(data_params["num_of_tests"]):
    ...
    def test_online_localization_correctness(self, inner_algorithm_factory, generate_data, data_params):
        for _ in range(data_params["num_of_tests"]):
            algorithm = inner_algorithm_factory()
            data = generate_data()
            for time, value in np.ndenumerate(data):
